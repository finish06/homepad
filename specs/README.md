...
| [`cap6-uptime-display-toggle.md`](./cap6-uptime-display-toggle.md) | Boolean toggle to show/hide the per-tile uptime sparkline. Default ON. **v1 shipped as a global admin System setting; v2 (2026-09-16) moves it to a per-user preference in My settings** — `users` column, `GET`/`PATCH /api/me`, seeded once from the global value so no view changes on migration. | **v1 shipped — prod v13.5.0 · v2 SHIPPED prod v16.4.0. Sign-offs waived under the owner standing rule; no outstanding gates.** |
