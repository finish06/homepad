...
|------|---------------|--------|
| [`SPEC-health-bar-visibility-toggle.md`](./SPEC-health-bar-visibility-toggle.md) | Per-user boolean that hides the health panel's distribution bar while keeping the verdict line. Column on `users`, read via `GET /api/me`, written via `PATCH /api/me` — the `themePref`/`densityPref` pattern. Default ON. | **Approved for build — OQ-1 resolved (a) 2026-09-15: SettingsPanel gains a `scope` prop; "My settings" in UserMenu opens it for all users. Kare §9 still unauthored.** |
...
| [`cap6-uptime-display-toggle.md`](./cap6-uptime-display-toggle.md) | Boolean toggle to show/hide the per-tile uptime sparkline. Default ON. **v1 shipped as a global admin System setting; v2 (2026-09-16) moves it to a per-user preference in My settings** — `users` column, `GET`/`PATCH /api/me`, seeded once from the global value so no view changes on migration. | **v1 shipped — prod v13.5.0 · v2 DRAFT, OQ-1 resolved (admin keeps a default for new accounts). Awaiting Walt + Kare §9 (AC-028 copy)** |
