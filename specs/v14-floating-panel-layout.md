...
| Hysteresis | **Swap categories only when the margin clears both ≥ 15% of the higher score AND ≥ 3 opens in the window** | Prevents churn from small statistical noise. The margin must clear the *larger* of the two thresholds at once — `Math.max(3, ceil(0.15 · higherScore))` (AND, not OR) |
...
| C-005 | Hysteresis: two categories swap positions only if the margin between them clears **both** ≥15% of the higher-scored category's 30-day opens **and** ≥3 opens — i.e. `margin ≥ Math.max(3, ceil(0.15 · higherScore))`. Below that margin, the current order is preserved. | Must |
