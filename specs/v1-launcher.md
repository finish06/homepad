...
| A5.1 | Both the per-tile reorder arrows **and the favorite ★ toggle** are hidden in the normal (decluttered) view and revealed together by a per-user **settings gear** button in the header — the non-admin settings/controls entry point, available to every logged-in user, not admin-gated. The gear hosts personal Arrange (reorder + favoriting) today and is the home for future per-user controls. Reordering and favoriting both stay personal and work for non-admins. **Only the star *toggle control* is gated** — favoriting still persists per-user (A5) and **favorited tiles still pin to the top section in the normal view**; the gear hides the editable control, never the favorites feature or the pinning. | Component: arrows + star absent by default, present when Arrange on; favorites stay pinned to the top in normal view regardless; non-admin can reorder and favorite with Arrange on (`PUT /api/layout`, `POST`/`DELETE /api/favorites/{id}`). Header: gear button (accessible label, e.g. "Personal settings") shown for both roles; activating it toggles arrange. |
...
3. **Personalization:** The header **settings gear** (any logged-in user) is the per-user controls entry point; activating it (Arrange mode) reveals both the per-tile up/down reorder arrows **and the favorite ★ toggle** — both hidden by default so the normal view stays clean; toggling off returns to the decluttered view. Favoriting and reordering persist per-user; favorited tiles stay pinned to the top section even in the normal view (the gear only hides the editable star, not the pinning). Future personal controls will live behind this same gear.
