...
// yields [] so the grid renders fully expanded — identical to v4 — rather
...
export async function getCollapsedCategories(): Promise<string[]> {
  const { status, res } = await request('/api/me/collapsed-categories');
  if (!res || status !== 200) return [];
  try {
...
export async function setCollapsedCategories(ids: string[]): Promise<boolean> {
  return boolRequest('/api/me/collapsed-categories', 204, { method: 'PUT', json: { collapsed: ids } });
}
